...
如果用户属于 sysadmin组,那么id参数将等于12/l(等于12);因此,应用程序返回介绍Syngress图书的页面。如果当前用户不是 sysadmin组的成员,那么id参数的值将为12/0(很明显不是数字);这将导致査询失败,应用返回一个错误。很明显,具体的错误消息会千差万别:  

可能只是一个由Web服务器返回的‘500 Internal server error',也可能包含完整的 SQL Server  
